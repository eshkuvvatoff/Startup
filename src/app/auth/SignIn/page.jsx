...
export default function SignInPage() {
  ...
  const oauthClass = "flex items-center cursor-pointer justify-center gap-2.5 py-3 rounded-xl border border-brand-violet/20 dark:border-white/8 bg-brand-violet/8 dark:bg-white/5 text-foreground hover:bg-brand-violet/15 dark:hover:bg-white/8 hover:border-brand-violet/40 dark:hover:border-white/15 hover:-translate-y-0.5 transition-all duration-200 shadow-sm";

  const inputClass = "w-full px-5 py-3.5 rounded-xl outline-none transition-all border border-brand-violet/40 dark:border-white/15 bg-brand-violet/5 dark:bg-white/5 text-foreground placeholder:text-muted-foreground/40 focus:border-brand-blue/70 dark:focus:border-brand-violet/60 focus:ring-4 focus:ring-brand-blue/10 dark:focus:ring-brand-violet/10";

  // const submitClass = "w-full py-3.5 rounded-xl cursor-pointer font-semibold text-sm text-white transition-all active:scale-[0.98] bg-gradient-to-r from-brand-blue via-brand-violet to-brand-blue shadow-[0_8px_25px_rgba(67,97,238,0.35)] dark:shadow-[0_8px_30px_rgba(67,97,238,0.45)] hover:opacity-90 hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center";
          ...
          <div className="backdrop-blur-2xl rounded-2xl p-7 bg-white/80 dark:bg-[rgba(10,8,40,0.75)] relative overflow-hidden transition-all duration-300">
            ...
            <div className="flex justify-center mb-6">
              <div className="flex items-center gap-2 px-3 py-1 rounded-full border border-brand-blue/20 dark:border-brand-cyan/15 bg-brand-blue/5 dark:bg-brand-cyan/5">
                ...
                <span className="text-[10px] font-semibold tracking-[0.15em] text-brand-blue dark:text-brand-cyan uppercase">Secure Access</span>
              </div>
            </div>

            {/* OAuth */}
            {/* OAuth - Chiroyli va individual uslubda */}
            <div className="grid grid-cols-3 gap-3 mb-8">

              {/* Google */}
              <button type="button" className="group relative flex h-12 items-center justify-center rounded-xl border border-brand-violet/20 bg-brand-violet/5 transition-all duration-300 hover:border-[#4285F4]  hover:bg-[#4285F4]/5">
                ...
                <svg className="relative cursor-pointer z-10 w-5 h-5" viewBox="0 0 24 24">
              ...
              {/* Apple */}
              <button type="button" className="group relative flex h-12 items-center justify-center rounded-xl border border-brand-violet/20 bg-brand-violet/5 transition-all duration-300 hover:border-[#4285F4] hover:bg-[#4285F4]/5">
                <div className="absolute inset-0 rounded-xl opacity-0 blur-md transition-opacity duration-300 group-hover:opacity-10 bg-[#4285F4] cursor-pointer" />
                <svg className="cursor-pointer relative z-10 w-5 h-5 text-foreground" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div className="relative mb-6 flex items-center">
              ...
              <span className="px-4 text-[10px] uppercase tracking-[0.25em] font-semibold text-muted-foreground/60 rounded-full bg-white/80 dark:bg-[rgba(10,8,40,0.75)]">or email</span>
              <div className="flex-1 border-t border-brand-violet/15 dark:border-white/8" />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5 group">
                ...
                <input required type="email" placeholder="name@aletheia.com" className={inputClass} />
              </div>

              <div className="space-y-1.5 group">
                <div className="flex justify-between items-center">
                  <label className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground transition-colors group-focus-within:text-brand-blue dark:group-focus-within:text-brand-cyan">
                    Password
                  </label>
                  <Link href="#" className="text-[11px] font-semibold text-brand-blue hover:text-brand-violet dark:text-brand-cyan dark:hover:text-brand-blue transition-colors">
                    Forgot?
                  </Link>
                </div>
                <div className="relative">
                  <input required type={showPassword ? "text" : "password"} placeholder="••••••••••" className={inputClass} />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute cursor-pointer right-4 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors">
            ...
            <p className="text-center mt-6 text-sm text-muted-foreground">
              No account?{" "}
              <Link href="/auth/SignUp" className="font-semibold text-foreground underline underline-offset-4 decoration-brand-violet hover:text-brand-violet transition-all">
                Sign up
              </Link>
            </p>
          </div>
